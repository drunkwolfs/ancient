...
(247,32-33): run-time error JS1195: Expected expression: >
(261,1-2): run-time error JS1002: Syntax error: }
(263,40-41): run-time error JS1004: Expected ';': {
(270,36-37): run-time error JS1195: Expected expression: >
(278,6-7): run-time error JS1195: Expected expression: )
(389,33-34): run-time error JS1195: Expected expression: >
(389,62-63): run-time error JS1004: Expected ';': )
(405,5-6): run-time error JS1002: Syntax error: }
(413,29-30): run-time error JS1195: Expected expression: >
(428,40-41): run-time error JS1195: Expected expression: )
(428,43-44): run-time error JS1195: Expected expression: >
(430,14-15): run-time error JS1195: Expected expression: ,
(431,9-10): run-time error JS1002: Syntax error: }
(441,5-6): run-time error JS1002: Syntax error: }
(444,32-33): run-time error JS1004: Expected ';': {
(449,1-2): run-time error JS1002: Syntax error: }
(454,57-58): run-time error JS1195: Expected expression: )
(454,59-60): run-time error JS1004: Expected ';': {
(460,27-28): run-time error JS1195: Expected expression: )
(460,30-31): run-time error JS1195: Expected expression: >
(460,64-65): run-time error JS1004: Expected ';': )
(477,35-36): run-time error JS1195: Expected expression: >
(482,8-9): run-time error JS1195: Expected expression: )
(484,62-63): run-time error JS1197: Too many errors. The file might not be a JavaScript file: (
(464,3-34): run-time error JS1301: End of file encountered before function is properly closed: function setupMobileNavToggle()
(484,67-68): run-time error JS1195: Expected expression: >
(484,69-70): run-time error JS1197: Too many errors. The file might not be a JavaScript file: {
 */
/*! modernizr 3.3.1 (Custom Build) | MIT *
 * https://modernizr.com/download/?-backgroundsize-boxshadow-cssanimations-csscolumns-csstransforms-csstransforms3d-csstransitions-flexbox-flexboxlegacy-fontface-generatedcontent-multiplebgs-domprefixes-hasevent-mq-prefixes-setclasses-shiv-testallprops-testprop-teststyles !*/
...
if ($('.stats_grid').length && !isMobileDevice()) {
    var hasAnimated = false;
    $(window).scroll(function () {
        var scrollTop = $(this).scrollTop();
        var counterOffset = $('.stats_grid').offset().top;
...
function closeAlertBanner(element, id) {
    var alertBanner = $(element).closest('.alert_banner');
    alertBanner.slideUp('fast');
    setCookie("hideAlert_" + id, "1", 7);
}

function closeOtherPanels(currentItem) {
    accordionItems.forEach((item) => {
        if (item !== currentItem) {
            const input = item.querySelector('input[type="checkbox"]');
            if (input != null) {
...
$('.nav_primary_btn').on('click', function (e) {
    toggleMobileNav(false);
});

$('.search_box input[type="radio"]').on('change', function () {
    var placeholder = $(this).data('placeholder');
    $('.search_box input[type="text"]').attr('placeholder', placeholder);
});

//Hide & Show Mobile Nav
function toggleMobileNav(hideOverride) {
    var mobileNav = $('.nav_primary > ul');
    if ($(mobileNav).is(':hidden') && !hideOverride) {
        $(mobileNav).slideDown('fast', function () {
            // Animation complete.
            $(mobileNav).addClass('show').removeAttr('style');
            $(".hamburger_button").addClass("toggled");
        });
    }
    else {
        $(mobileNav).slideUp('fast', function () {
            ...
            $(".hamburger_button").removeClass("toggled");
        });
    }
}

//Click outside of the mobile nav to the close mobile nav
$(document).on('click', function (e) {
    if ($(".nav_primary_btn").is(':visible') //If the hamburger button is visible
        && $('.nav_primary > ul').is(':visible') //If the mobile nav is visible
        && !$(e.target).is('.nav_primary_btn') //If the element being clicked is not the hamburger button
        && !$(e.target).is('.nav_primary') //If the element being clicked is not the nav
        && !$(e.target).parents('.nav_primary_btn').length > 0 //If the element being clicked is not a child of the hamburger button
        && !$(e.target).parents('.nav_primary').length > 0 //If the element being clicked is not a child of the nav
    ) {
...
$(document).ready(setupHoverIntent);
$(window).on("resize", setupHoverIntent);
...
// Mega Menu (Platform) Refresh
document.addEventListener("DOMContentLoaded", function () {
    const megaItems = document.querySelectorAll(".mega-items li");
    const megaLists = document.querySelectorAll(".mega-lists > div");
    const parent = document.querySelector(".parent");
    let activeMegaList = null;
    let activeMegaItem = null;
    let hoverTimeout;

    function showMegaItem(item) {
        // Hide all mega lists
        megaLists.forEach(list => list.style.display = "none");

        // Remove active class from previous item
        if (activeMegaItem) {
            activeMegaItem.classList.remove("mega-item-active");
        }

        // Show the corresponding mega list
        const targetClass = item.classList[0];
        const targetList = document.querySelector(".mega-lists ." + targetClass);
        if (targetList) {
            targetList.style.display = "flex";
            activeMegaList = targetList;
            activeMegaItem = item;
            activeMegaItem.classList.add("mega-item-active");
        }
    }

    // Set mega-platform active by default
    const defaultItem = document.querySelector(".mega-items .mega-platform");
    if (defaultItem) {
        showMegaItem(defaultItem);
    }

    megaItems.forEach(item => {
        item.addEventListener("mouseenter", function () {
            // Ignore if it's not one of the allowed mega items
            if (
                !item.classList.contains("mega-platform") &&
                !item.classList.contains("mega-ams") && 
                !item.classList.contains("mega-commissions") && 
                !item.classList.contains("mega-quote") &&
                !item.classList.contains("mega-engage")
            ) {
                return;
            }
    ...
    // Keep the active mega list visible when hovered
    megaLists.forEach(list => {
        list.addEventListener("mouseenter", function () {
            if (activeMegaList === list) {
                activeMegaList.style.display = "flex";
            }
        });
    });

    // Reset to default when leaving the parent container
    if (parent && defaultItem) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setupMobileNavToggle() {
    const nav = document.querySelector("nav.nav_primary");
    if (!nav) return;

    const parentItems = nav.querySelectorAll(
      "ul.parent > li.drop_nav > a, ul.parent > li.drop_mega > a"
    );
    ...
    if (!isMobile) {
      // Desktop: remove mobile click handlers and collapse any mobile-open state
      parentItems.forEach(anchor => {
        if (anchor._clickHandler) {
          anchor.removeEventListener("click", anchor._clickHandler);
          anchor._clickHandler = null;
        }
      });

      nav.querySelectorAll("ul.parent > li.nav_open").forEach(li => {
        li.classList.remove("nav_open");
      });
      nav.querySelectorAll("ul.parent > li .show_drop_nav").forEach(ul => {
        ul.classList.remove("show_drop_nav");
      });
    ...
    parentItems.forEach(anchor => {
      ...
      const isDropNav = parentLi.classList.contains("drop_nav");

      if (isDropNav) {
        dropNavCount++;
        if (dropNavCount === 3) return; // skip the 3rd drop_nav
      }
      ...
      // Keep original href but stop navigation
      ...
      anchor._clickHandler = function (e) {
        ...
        let localDropNavCount = 0;
        nav.querySelectorAll("ul.parent > li.drop_nav, li.drop_mega").forEach(li => {
          const liIsDropNav = li.classList.contains("drop_nav");
          if (liIsDropNav) {
            localDropNavCount++;
            if (localDropNavCount === 3) return;
          }
          if (li !== parentLi) {
            li.classList.remove("nav_open");
            const ul = li.querySelector("ul");
            if (ul) ul.classList.remove("show_drop_nav");
          }
        });

        // Toggle clicked menu (CSS rotation is driven by .nav_open)
        if (subMenu && subMenu.tagName === "UL") {
          subMenu.classList.toggle("show_drop_nav");
    ...
    // Ensure a down-angle icon exists on each parent link
    let iconDropNavCount = 0;
    nav.querySelectorAll("ul.parent > li > a").forEach(anchor => {
      const parentLi = anchor.parentElement;
      if (parentLi.classList.contains("drop_nav")) {
        iconDropNavCount++;
        if (iconDropNavCount === 3) return;
      }

      // Don't duplicate if any fa-angle-down exists, regardless of weight (regular/solid)
      if (!anchor.querySelector('i[class*="fa-angle-down"]')) {
        const icon = document.createElement("i");
        icon.className = "fa-regular fa-angle-down"; // works with FA5/FA6 webfonts
        anchor.appendChild(icon);
      }
    });
  }

  const debouncedSetup = debounce(setupMobileNavToggle, 150);
...
function Tabs(args) {
    ...
    var toggle = function () {
        ...
        // Scroll to the target section if screen size is 640px or less
